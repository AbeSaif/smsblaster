...
const LandlineSchema = mongoose.Schema(
  {
    directImportId: {
      type: String,
    },
...
LandlineSchema.index({ msgDate: 1, status: 1, status2: 1, status3: 1 });
LandlineSchema.index({
  msgDate: 1,
  campaignId1: 1,
  campaignId2: 1,
  campaignId3: 1,
...
LandlineSchema.index({ delivered3: 1,status3: 1, undelivered: 1, msgDate3: 1 });
LandlineSchema.index({delivered2: 1, status2: 1,msgDate2: 1,bandwithsendid1: 1});

LandlineSchema.index({ phone1: 1, phone2: 1, phone3: 1 });
LandlineSchema.index({ directImportId: 1 });
LandlineSchema.index({
  directImportId: 1,
  msgDate: 1,
  msgDate2: 1,
...
LandlineSchema.index({ marketSenderNumber: 1 });
LandlineSchema.index({ bandwithsendid1: 1 });
LandlineSchema.index({ bandwithsendid2: 1 });
LandlineSchema.index({ bandwithsendid3: 1 });
LandlineSchema.index({ phone2: 1 });
LandlineSchema.index({ phone3: 1 });
LandlineSchema.index({ batchId: 1, phone2: 1 });
LandlineSchema.index({ batchId: 1, phone3: 1 });
...
LandlineSchema.index({ propertyAddress: 1 }, { collation: collationOptions });
LandlineSchema.index({ propertyCity: 1 }, { collation: collationOptions });
LandlineSchema.index({ propertyState: 1 }, { collation: collationOptions });
LandlineSchema.index({ propertyZip: 1 }, { collation: collationOptions });
// add plugin that converts mongoose to json
LandlineSchema.plugin(toJSON);
LandlineSchema.plugin(paginate);

const Landline = mongoose.model("Landline", LandlineSchema);
