...
//         const existingPhoneNumber = callback.message.from.replace(/^\+1/, "");
//         const filter = {
//           $or: [
//             {
//               $and: [
//                 { to: existingPhoneNumber },
//                 { "messages.bandWidthId": { $ne: callback.message.id } },
//                 { isWrongNumber: false },
//                 { isAddedToDNC: false },
//               ],
//             },
...
//                 { "messagesPhone2.bandWidthId": { $ne: callback.message.id } },
...
//                 { "messagesPhone3.bandWidthId": { $ne: callback.message.id } },
...
//           isOutgoing: false,
//           isViewed: false,
//           bandWidthId: callback.message.id,
//         };
...
//         if (callback.message.tag !== "undefined") {
//           PhoneNumber = callback.to.replace(/^\+1/, "");
//           const valueToCheck = callback.message.id;
//           const aggregatedData = await CsvData.aggregate([
//             {
//               $match: {
//                 $or: [
...
//                   $switch: {
//                     branches: [
//                       { case: { $eq: ["$bandwithsendid1", valueToCheck] }, then: "bandwithsendid1" },
//                       { case: { $eq: ["$bandwithsendid2", valueToCheck] }, then: "bandwithsendid2" },
//                       { case: { $eq: ["$bandwithsendid3", valueToCheck] }, then: "bandwithsendid3" },
//                     ],
//                     default: "None",
//                   },
//                 },
...
//             // Define the matching field and delivery update
//             switch (matchingField) {
//               case "bandwithsendid1":
//                 deliveryUpdate = { delivered: 1 };
//                 foundDeliverd = await CsvData.find({ bandwithsendid1: valueToCheck, delivered: 1 });
//                 break;
//               case "bandwithsendid2":
//                 deliveryUpdate = { delivered2: 1 };
//                 foundDeliverd = await CsvData.find({ bandwithsendid2: valueToCheck, delivered2: 1 });
//                 break;
//               case "bandwithsendid3":
//                 deliveryUpdate = { delivered3: 1 };
//                 foundDeliverd = await CsvData.find({ bandwithsendid3: valueToCheck, delivered3: 1 });
//                 break;
//               default:
//                 return "Nothing";
//             }
...
//               if (updatedCsvData) {
//                 console.log("Updated CsvData.directImportId:", updatedCsvData.directImportId);

//                 let campaignId;

//                 // Find the campaignId from the updatedCsvData
//                 if (updatedCsvData.campaignId) {
//                   campaignId = updatedCsvData.campaignId;
//                 } else {
//                   // Check other campaignId fields if needed
//                   campaignId = updatedCsvData.campaignId1 || updatedCsvData.campaignId2 || updatedCsvData.campaignId3;
//                 }

//                 if (campaignId) {
//                   // Update the Campaign and DirectImport within a transaction
...
//                   const [updatedCampaign, updatedDirectImport] = await Promise.all([campaignUpdate, directImportUpdate]);
...
//                     throw new Error(`DirectImport with ID ${updatedCsvData.directImportId} not found.`);
//                   }
//                 } else {
//                   console.log("No campaign ID found to increment delivered value.");
//                 }
//               } else {
...
//           PhoneNumber = callback.to.replace(/^\+1/, "");
//           const valueToCheck = callback.message.id;
//           await Flag.deleteMany({ to: callback.to });
//           const aggregatedData = await CsvData.aggregate([
//             {
//               $match: {
//                 $or: [
...
//                     const newDeliveredValue =
//                       (parseInt(campaign.totalDelivered) || 0) + 1;
//                       console.log("newDeliveredValue",newDeliveredValue);
//                     // Update the campaign with the new delivered value
//                     await Compaign.updateOne(
//                       { _id: campaign._id },
//                       { totalDelivered: newDeliveredValue }
//                     );

//                     const directImport = await DirectImport.findOne({ _id: updatedCsvData.directImportId });
//                     const newDeliveredValueDI =
//                       (parseInt(directImport.delivered) || 0) + 1;

//                     const updateDirectImport = await DirectImport.findOneAndUpdate(
//                       { _id: updatedCsvData.directImportId },  // Find by assignedCampaign
//                       { delivered: newDeliveredValueDI},              // Increment delivered by 1
//                       { new: true }                            // Return the updated document
//                     )
//                     console.log("newDeliveredValueDI",newDeliveredValueDI);
//                     console.log(
...
//                   }
//                   console.log(
//                     `incrementDeliveredValue called for campaignId: ${campaignId}`
//                   );
//                 } catch (err) {
//                   console.error(
//                     `Failed to increment delivered value for campaignId: ${campaignId}`,
//                     err
//                   );
//                 }
//               } else {
...
//           $and: [
//             { to: callback?.to },
//             { "message.from": callback?.message?.from },
//           ],
//         });
//         if (!flagRecordExist) {
...
// async function incrementDelivered(campaignId) {
//   try {
//     const campaign = await Compaign.findOne({ _id: campaignId });
//     if (!campaign) {
//       throw new Error(`Campaign with ID ${campaignId} not found.`);
...
//     // Increment totalDelivered in Campaign
//     campaign.totalDelivered += 1;

//     // Update DirectImport
//     const directImport = await DirectImport.findOne({ assignCampaign: campaignId });
//     if (!directImport) {
//       throw new Error(`DirectImport with Campaign ID ${campaignId} not found.`);
//     }
//     directImport.delivered += 1;

//     // Save both changes
//     await campaign.save();
//     await directImport.save();

//     console.log(`Consistency maintained: Campaign ID ${campaignId}`);
//   } catch (err) {
//     console.error(`Failed to increment delivered counts:`, err);
//   }
// }
const smsStatus = catchAsync(async (req, res) => {
  try {
...
const smsStatus1 = catchAsync(async (req, res) => {
  try {
    const callback = req.body[0];
    let PhoneNumber;
    let updateSql;
